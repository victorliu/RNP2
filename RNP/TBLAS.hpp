#ifndef RNP_TBLAS_HPP_INCLUDED
#define RNP_TBLAS_HPP_INCLUDED
...
void Scale(const S &scale, Vector<T> &x){
	for(size_t i = 0; i < x.size(); ++i){
		x[i] *= scale;
	}
}
...
template <typename S, typename T>
void Scale(const S &scale, Matrix<T> &a){
	for(size_t j = 0; j < a.cols(); ++j){
		Scale(a.Column(j), scale);
	}
}
...
void Copy(const Matrix<T> &src, Matrix<T> &dst){
	...
	for(size_t j = 0; j < dst.cols(); ++j){
		Vector<T> dstcol = dst.Column(j);
		Copy(src.Column(j), dstcol);
	}
}

template <class S, class T>
void Axpy(const S &alpha, const Vector<T> &x, Vector<T> &y){
	RNPAssert(x.size() == y.size());
...
template <class S, class T>
void Axpy(const S &alpha, const Matrix<T> &x, Matrix<T> &y){
	RNPAssert((x.rows() == y.rows()) && (x.cols() == y.cols()));
	for(size_t j = 0; j < y.cols(); ++j){
		Vector<T> ycol = x.Column(j);
		Axpy(alpha, x.Column(j), ycol);
	}
}
...
template <class T>
T ConjugateDot(const Vector<T> &x, const Vector<T> &y){
	RNPAssert(x.size() == y.size());
	T sum(0);
	for(size_t i = 0; i < x.size(); ++i){
		sum += RNP::Traits<T>::conj(x[i]) * y[i];
	}
	return sum;
}

template <class T>
typename RNP::Traits<T>::Real Norm2(const Vector<T> &x){
	typedef typename RNP::Traits<T>::Real real_type;
	typedef typename RNP::Traits<real_type> real_traits;
	real_type ssq(1), scale(0);
	for(size_t i = 0; i < x.size(); ++i){
		if(0 != RNP::Traits<T>::real(x[i])){
			real_type temp = real_traits::abs(RNP::Traits<T>::real(x[i]));
			if(scale < temp){
		...
		if(0 != RNP::Traits<T>::imag(x[i])){
			real_type temp = real_traits::abs(RNP::Traits<T>::imag(x[i]));
			if(scale < temp){
				real_type r = scale/temp;
				ssq = ssq*r*r + real_type(1);
				scale = temp;
...
template <class T>
typename RNP::Traits<T>::Real Asum(const Vector<T> &x){
	typename RNP::Traits<T>::Real sum(0);
	for(size_t i = 0; i < x.size(); ++i){
		sum += RNP::Traits<T>::abs1(x[i]);
	}
	return sum;
}

template <class T>
size_t MaximumIndex(const Vector<T> &x){
	RNPAssert(x.size() > 0);
	typedef typename RNP::Traits<T>::Real real_type;
	real_type mv = RNP::Traits<T>::abs1(x[0]);
	size_t mi = 0;
	for(size_t i = 1; i < x.size(); ++i){
		real_type cv = RNP::Traits<T>::abs1(x[i]);
		if(cv > mv){ mi = i; mv = cv; }
	}
	return mi;
}
			...
			if('U' == uplo){
				...
				while(j --> 0){
					...
					if(noconj){
						...
						if(nounit){ temp *= RNP::RealOrComplexChooser<T>::conj(a[j+j*lda]); }
						size_t i = j-1;
						while(i --> 0){
							x2 -= incx;
							temp += RNP::RealOrComplexChooser<T>::conj(a[i+j*lda])*(*x2);
						}
					}
					*xx = temp;
					xx -= incx;
	...
	MultHermV(size_t n, const TA &alpha, const T *a, size_t lda, const T *x, size_t incx, const TB &beta, T *y, size_t incy){
		...
		if(uplo == 'U'){ // Form y when A is stored in upper triangle.
			...
			for(size_t j =  0; j < n; ++j){
				...
				for(size_t i = 0; i < j; ++i){
					*yi += temp1*a[i+j*lda];
					temp2 += RNP::RealOrComplexChooser<T>::conj(a[i+j*lda])*(*xi);
					xi += incx;
					yi += incy;
				}
				*yj += temp1*RNP::RealOrComplexChooser<T>::real(a[j+j*lda]) + alpha*temp2;
				xj += incx;
				yj += incy;
			}
			...
			for(size_t j = 0; j < n; ++j){
				T temp1 = alpha*(*xj);
				T temp2 = 0;
				*yj += temp1*RNP::RealOrComplexChooser<T>::real(a[j+j*lda]);
				const T *xi = xj+1;
				T *yi = yj+1;
				for(size_t i = j+1; i < n; ++i){
					*yi += temp1*a[i+j*lda];
			...
			if(uplo == 'U'){
				...
				for(size_t j = 0; j < n; ++j){
					...
					if(noconj){
						...
						for(size_t i = 0; i < j; ++i){
							temp -= RNP::RealOrComplexChooser<T>::conj(a[i+j*lda]) * *xix;
							xix += incx;
						}
						if(nounit){
							temp /= RNP::RealOrComplexChooser<T>::conj(a[j+j*lda]);
						}
					}
					*xjx = temp;
					xjx += incx;
...
void ConjugateRank1Update(size_t m, size_t n, const A &alpha, const T *x, size_t incx, const T *y, size_t incy, T *a, size_t lda){
	...
	for(size_t j = 0; j < n; ++j){
		if(T(0) != *y){
			T temp = alpha*RNP::RealOrComplexChooser<T>::conj(*y);
			const T *xx = x;
			for(size_t i = 0; i < m; ++i){
				a[i+j*lda] += (*xx)*temp;
				xx += incx;
		...
		if(notb){
			if(nota){ // Form  C := alpha*A*B + beta*C.
				...
				for(size_t j = 0; j < n; ++j){
					for(size_t i = 0; i < m; ++i){
						T temp = 0;
						for(size_t l = 0; l < k; ++l){
							temp += RNP::RealOrComplexChooser<T>::conj(a[l+i*lda]) * b[l+j*ldb];
						}
						if(B(0) == beta){
			...
			if(conjb){ // Form  C := alpha*A*conjg( B' ) + beta*C.
				for(size_t j = 0; j < n; ++j){
					...
					for(size_t l = 0; l < k; ++l){
						if(T(0) != b[j+l*ldb]){
							T temp = alpha * RNP::RealOrComplexChooser<T>::conj(b[j+l*ldb]);
							for(size_t i = 0; i < m; ++i){
								c[i+j*ldc] += temp * a[i+l*lda];
							}
						}
			...
			if(conjb){ // Form  C := alpha*conjg( A' )*conjg( B' ) + beta*C.
				for(size_t j = 0; j < n; ++j){
					for(size_t i = 0; i < m; ++i){
						T temp = 0;
						for(size_t l = 0; l < k; ++l){
							temp += RNP::RealOrComplexChooser<T>::conj(a[l+i*lda]) * RNP::RealOrComplexChooser<T>::conj(b[j+l*ldb]);
						}
						if(B(0) == beta){
							c[i+j*ldc] = alpha * temp;
						}else{
				...
				for(size_t j = 0; j < n; ++j){
					for(size_t i = 0; i < m; ++i){
						T temp = 0;
						for(size_t l = 0; l < k; ++l){
							temp += RNP::RealOrComplexChooser<T>::conj(a[l+i*lda]) * b[j+l*ldb];
						}
						if(B(0) == beta){
							c[i+j*ldc] = alpha * temp;
						}else{
			...
			if(conjb){ // Form  C := alpha*A'*conjg( B' ) + beta*C
				for(size_t j = 0; j < n; ++j){
					for(size_t i = 0; i < m; ++i){
						T temp = 0;
						for(size_t l = 0; l < k; ++l){
							temp += a[l + i * lda] * RNP::RealOrComplexChooser<T>::conj(b[j+l*ldb]);
						}
						if(B(0) == beta){
							c[i+j*ldc] = alpha * temp;
						}else{
					...
					for(size_t j = 0; j < n; ++j){
						for(size_t i = 0; i < m; ++i){
							...
							if(noconj){
								...
								for(size_t k = 0; k < i; ++k){
									temp -= RNP::RealOrComplexChooser<T>::conj(a[k+i*lda]) * b[k+j*ldb];
								}
								if(nounit){
									temp /= RNP::RealOrComplexChooser<T>::conj(a[i+i*lda]);
								}
							}
							b[i+j*ldb] = temp;
						}
					...
					for(int k = (int)n-1; k >= 0; --k){
						...
						if(nounit){
							if(noconj){
								...
								temp = T(1) / RNP::RealOrComplexChooser<T>::conj(a[k+k*lda]);
							}
							for(size_t i = 0; i < m; ++i){
						...
						for(size_t j = 0; j < (size_t)k; ++j){
							if(T(0) != a[j+k*lda]){
								if(noconj){
									...
									temp = RNP::RealOrComplexChooser<T>::conj(a[j+k*lda]);
								}
								for(size_t i = 0; i < m; ++i){
									b[i+j*ldb] -= temp * b[i+k*ldb];
								}
...
void Conjugate(size_t n, T *x, size_t incx){ // zlacgv, clacgv
	while(n --> 0){
		*x = RNP::RealOrComplexChooser<T>::conj(*x);
		x += incx;
	}
}
...
struct MultDiagM{
	...
	MultDiagM(size_t m, size_t n, const TD *d, size_t dinc, TA *a, size_t lda){
		if('L' == side){
			for(size_t i = 0; i < m; ++i){
				Scale(n, *d, a[i],lda);
				d += dinc;
