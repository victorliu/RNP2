#ifndef RNP_HESSENBERG_QR_HPP_INCLUDED
#define RNP_HESSENBERG_QR_HPP_INCLUDED
...
int SchurReduce(
	...
	const size_t nmin = Tuning<T>::multishift_crossover_size(wantt, wantz, n, ilo, ihi);

	// small bulge multishift for big matrices; implicit single-shift for small ones
	if(n >= nmin){
		info = ComplexSmallBulgeMultishiftQR(
			0, wantt, wantz, n, ilo, ihi, h, ldh, w, ilo, ihi, z, ldz, lwork, work
		);
	}else{ // Small matrix
		info = ComplexImplicitSingleShiftQR(
...
#endif // RNP_HESSENBERG_QR_HPP_INCLUDED
