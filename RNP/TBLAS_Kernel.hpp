#ifndef RNP_TBLAS_KERNEL_HPP_INCLUDED
#define RNP_TBLAS_KERNEL_HPP_INCLUDED
...
static void Fill(size_t n, const TV &value, T *x, size_t incx){
	while(n --> 0){
		*x = value;
		x += incx;
	}
...
template <class T>
static void Swap(size_t n, T *x, size_t incx, T *y, size_t incy){
	while(n --> 0){
		std::swap(*x, *y);
		x += incx; y += incy;
	}
...
static void Scale(size_t n, const S &scale, T *x, size_t incx){
	while(n --> 0){
		(*x) *= scale;
		x += incx;
	}
...
template <class S, class T>
static void Scale(size_t m, size_t n, const S &scale, T *a, size_t lda){
...
template <class T>
static void Copy(size_t n, const T *src, size_t incsrc, T *dst, size_t incdst){
	while(n --> 0){
		(*dst) = (*src);
...
template <class S, class T>
static void Axpy(size_t n, const S &alpha, const T *x, size_t incx, T *y, size_t incy){
	while(n --> 0){
		(*y) += alpha*(*x);
		x += incx; y += incy;
	}
...
template <class S, class T>
static void Axpy(size_t m, size_t n, const S &alpha, const T *x, size_t ldx, T *y, size_t ldy){
	for(size_t j = 0; j < n; ++j){
...
template <class T>
static T Dot(size_t n, const T *x, size_t incx, const T *y, size_t incy){
	T sum(0);
	while(n --> 0){
		sum += (*x)*(*y);
		x += incx; y += incy;
...
template <class T>
static T ConjugateDot(size_t n, const T *x, size_t incx, const T *y, size_t incy){
	T sum(0);
	while(n --> 0){
		sum += RNP::Traits<T>::conj(*x)*(*y);
		x += incx; y += incy;
	}
	return sum;
}
...
static typename RNP::Traits<T>::real_type Norm2(size_t n, const T *x, size_t incx){
	...
	while(n --> 0){
		if(0 != RNP::Traits<T>::real(*x)){
			real_type temp = abs(RNP::Traits<T>::real(*x));
			if(scale < temp){
		...
		if(0 != RNP::Traits<T>::imag(*x)){
			real_type temp = abs(RNP::Traits<T>::imag(*x));
			if(scale < temp){
				real_type r = scale/temp;
				ssq = ssq*r*r + real_type(1);
				scale = temp;
...
// zgemv, dgemv, cgemv, sgemv
template <class A, class B, class T>
static void MultMV(char trans, size_t m, size_t n, const A &alpha, const T *a, size_t lda,
	...
	if('N' == trans){ // y <- alpha*A*x + y
		...
		for(size_t j = 0; j < n; ++j){
			...
			if(noconj){
				...
				const T *xx = x;
				for(size_t i = 0; i < m; ++i){
					temp += RNP::Traits<T>::conj(a[i+j*lda])*(*xx);
					xx += incx;
				}
			}
			*y += alpha*temp;
...
// ztrmv, dtrmv, ctrmv, strmv
template <class T>
static void MultTrV(
	char uplo, char trans, char diag,
	size_t n, const T *a, size_t lda, T *x, size_t incx
){
	if(n < 1){ return; }
		...
		if('U' == uplo){
			...
			while(j --> 0){
				...
				if(noconj){
					...
					if(nounit){ temp *= RNP::Traits<T>::conj(a[j+j*lda]); }
					size_t i = j-1;
					while(i --> 0){
						x2 -= incx;
						temp += RNP::Traits<T>::conj(a[i+j*lda])*(*x2);
					}
				}
				*xx = temp;
				xx -= incx;
...
// zhemv, chemv, dsymv, ssymv
template <class TA, class TB, class T>
static void MultHermV(
	char uplo,
	size_t n, const TA &alpha, const T *a, size_t lda,
	const T *x, size_t incx,
	const TB &beta, T *y, size_t incy
){
	// 
	...
	if(uplo == 'U'){ // Form y when A is stored in upper triangle.
		...
		for(size_t j =  0; j < n; ++j){
			...
			for(size_t i = 0; i < j; ++i){
				*yi += temp1*a[i+j*lda];
				temp2 += RNP::Traits<T>::conj(a[i+j*lda])*(*xi);
				xi += incx;
				yi += incy;
			}
			*yj += temp1*RNP::Traits<T>::real(a[j+j*lda]) + alpha*temp2;
			xj += incx;
			yj += incy;
		}
		...
		for(size_t j = 0; j < n; ++j){
			T temp1 = alpha*(*xj);
			T temp2 = 0;
			*yj += temp1*RNP::Traits<T>::real(a[j+j*lda]);
			const T *xi = xj+1;
			T *yi = yj+1;
			for(size_t i = j+1; i < n; ++i){
				*yi += temp1*a[i+j*lda];
...
// ztrsv, dtrsv, ctrsv, strsv
template <class T>
static void SolveTrV(char uplo, char trans, char diag, size_t n, const T *a, size_t lda, T *x, size_t incx){
		...
		if(uplo == 'U'){
			...
			for(size_t j = 0; j < n; ++j){
				...
				if(noconj){
					...
					for(size_t i = 0; i < j; ++i){
						temp -= RNP::Traits<T>::conj(a[i+j*lda]) * *xix;
						xix += incx;
					}
					if(nounit){
						temp /= RNP::Traits<T>::conj(a[j+j*lda]);
					}
				}
				*xjx = temp;
				xjx += incx;
...
// MultTrBandedV
// SolveTrBandedV
// Note: Packed routines are very low priority

template <class A, class T> // zgeru, dgeru, cgeru, sgeru
static void Rank1Update(size_t m, size_t n, const A &alpha, const T *x, size_t incx, const T *y, size_t incy, T *a, size_t lda){
...
static void ConjugateRank1Update(size_t m, size_t n, const A &alpha, const T *x, size_t incx, const T *y, size_t incy, T *a, size_t lda){
	// performs A += alpha*x*conj(y')
	if(m < 1 || n < 1 || A(0) == alpha){ return; }
	for(size_t j = 0; j < n; ++j){
		if(T(0) != *y){
			T temp = alpha*RNP::Traits<T>::conj(*y);
			const T *xx = x;
			for(size_t i = 0; i < m; ++i){
				a[i+j*lda] += (*xx)*temp;
				xx += incx;
...
// MultMM (zgemm)
template <class A, class B, class T>
static void MultMM(char transa, char transb,
	size_t m, size_t n, size_t k, const A &alpha, const T *a, size_t lda,
	...
	if(notb){
		if(nota){ // Form  C := alpha*A*B + beta*C.
			...
			for(size_t j = 0; j < n; ++j){
				for(size_t i = 0; i < m; ++i){
					T temp = 0;
					for(size_t l = 0; l < k; ++l){
						temp += RNP::Traits<T>::conj(a[l+i*lda]) * b[l+j*ldb];
					}
					if(B(0) == beta){
		...
		if(conjb){ // Form  C := alpha*A*conjg( B' ) + beta*C.
			for(size_t j = 0; j < n; ++j){
				...
				for(size_t l = 0; l < k; ++l){
					if(T(0) != b[j+l*ldb]){
						T temp = alpha * RNP::Traits<T>::conj(b[j+l*ldb]);
						for(size_t i = 0; i < m; ++i){
							c[i+j*ldc] += temp * a[i+l*lda];
						}
					}
		...
		if(conjb){ // Form  C := alpha*conjg( A' )*conjg( B' ) + beta*C.
			for(size_t j = 0; j < n; ++j){
				for(size_t i = 0; i < m; ++i){
					T temp = 0;
					for(size_t l = 0; l < k; ++l){
						temp += RNP::Traits<T>::conj(a[l+i*lda]) * RNP::Traits<T>::conj(b[j+l*ldb]);
					}
					if(B(0) == beta){
						c[i+j*ldc] = alpha * temp;
					}else{
			...
			for(size_t j = 0; j < n; ++j){
				for(size_t i = 0; i < m; ++i){
					T temp = 0;
					for(size_t l = 0; l < k; ++l){
						temp += RNP::Traits<T>::conj(a[l+i*lda]) * b[j+l*ldb];
					}
					if(B(0) == beta){
						c[i+j*ldc] = alpha * temp;
					}else{
		...
		if(conjb){ // Form  C := alpha*A'*conjg( B' ) + beta*C
			for(size_t j = 0; j < n; ++j){
				for(size_t i = 0; i < m; ++i){
					T temp = 0;
					for(size_t l = 0; l < k; ++l){
						temp += a[l + i * lda] * RNP::Traits<T>::conj(b[j+l*ldb]);
					}
					if(B(0) == beta){
						c[i+j*ldc] = alpha * temp;
					}else{
...
// ztrmm, dtrmm, ctrmm, strmm
template <class T, class TA>
static void MultTrM(
	char side, char uplo, char transa, char diag,
	size_t m, size_t n, const TA &alpha, const T *a, size_t lda, T *b, size_t ldb
){
	// Purpose
	// =======
	// 
...
// ztrsm, dtrsm, ctrsm, strsm
template <class TA, class T>
static void SolveTrM(
	char side, char uplo, char transa, char diag,
	size_t m, size_t n, const TA &alpha, const T *a, size_t lda, T *b, size_t ldb
){
				...
				for(size_t j = 0; j < n; ++j){
					for(size_t i = 0; i < m; ++i){
						...
						if(noconj){
							...
							if(nounit){
								temp /= RNP::Traits<T>::conj(a[i+i*lda]);
							}
						}
						b[i+j*ldb] = temp;
					}
				...
				for(int k = (int)n-1; k >= 0; --k){
					...
					if(nounit){
						if(noconj){
							...
							temp = T(1) / RNP::Traits<T>::conj(a[k+k*lda]);
						}
						for(size_t i = 0; i < m; ++i){
					...
					for(size_t j = 0; j < (size_t)k; ++j){
						if(T(0) != a[j+k*lda]){
							if(noconj){
								...
								temp = RNP::Traits<T>::conj(a[j+k*lda]);
							}
							for(size_t i = 0; i < m; ++i){
								b[i+j*ldb] -= temp * b[i+k*ldb];
							}
...
template <class T>
static void Conjugate(size_t n, T *x, size_t incx){ // zlacgv, clacgv
	while(n --> 0){
		*x = RNP::Traits<T>::conj(*x);
		x += incx;
	}
}

// zlacpy, dlacpy, clacpy, slacpy
template <class T>
static void CopyMatrix(char uplo, size_t m, size_t n, const T *a, size_t lda, T *b, size_t ldb){
	if('U' == uplo){
		for(size_t j = 0; j < n; ++j){
...
// zlaset, dlaset, claset, slaset
template <class TA, class TB, class T>
static void SetMatrix(char uplo, size_t m, size_t n, const TA &offdiag, const TB &diag, T *a, size_t lda){
...
// Multiplies a matrix A by a diagonal matrix from the left (row scaling)
// or from the right (column scaling).
template <class TA, class TD>
static void MultDiagM(char side, size_t m, size_t n, const TD *d, size_t dinc, TA *a, size_t lda){
	if('L' == side){
		for(size_t i = 0; i < m; ++i){
			Scale(n, *d, a[i],lda);
			d += dinc;
...
#endif // RNP_TBLAS_KERNEL_HPP_INCLUDED
